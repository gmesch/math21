...
function initslides() {
  ...
  if (params['follow']) {
    const receiver = new FirebaseReceiver(params['follow']);
    installStateReceiver(receiver);
    installCanvasReceiver(receiver, canvas);
  }

  if (params['lead']) {
    const sender = new FirebaseSender(params['lead']);
    installStateSender(sender);
    installCanvasSender(sender, canvas);
  }
}

function installStateReceiver(receiver) {
  receiver.onMessage(function(data) {
    if (data.dispatch == 'state') {
      window.location.assign('#' + data.state);
    }
  });
}

function installStateSender(sender) {
  let currentState = '';
  window.setInterval(function() {
    if (window.location.hash != currentState) {
      currentState = window.location.hash;
      sender.send({dispatch: 'state', state: currentState.substr(1)});
    }
  }, 100);
}
