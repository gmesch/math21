...
Slides.prototype.processSteps_ = function(active, previous, opt_step) {
  ...
  if (!domTestClass(slide, 'build') &&
      !domTestClass(slide, 'build-visible') &&
      !domTestClass(slide, 'build-focus')) {
    this.steps_ = 0;
    this.step_ = 0;
    return;
  }
...
Slides.prototype.processStep_ = function() {
  ...
  const focus = domTestClass(slide, 'build-focus');
  const visible = focus || domTestClass(slide, 'build-visible');
