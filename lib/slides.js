...
Slides.prototype.setActiveImpl_ = function(a, opt_step) {
  this.processAttribute_(this.getSlideElement_(this.slide_), 'onhide');
  ...
  this.processAttribute_(this.getSlideElement_(this.slide_), 'onshow');
...
Slides.prototype.processFloater_ = function(slideNumber, show) {
  const slideElement = this.getSlideElement_(slideNumber);
  if (!slideElement) {
    return;
  }
...
Slides.prototype.processSteps_ = function(active, previous, opt_step) {
  ...
  const slideElement = this.getSlideElement_(active);
  if (!slideElement) {
    return;
  }
  ...
  } else if (active in this.stepBySlide_) {
    this.step_ = this.stepBySlide_[active];
...
Slides.prototype.processStep_ = function() {
  ...
  const focus = domTestClass(slideElement, 'build-focus');
  const visible = focus || domTestClass(slideElement, 'build-visible');
